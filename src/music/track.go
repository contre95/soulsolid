...
// ValidateImport validates the track fields for import, optionally allowing missing artist/album metadata.
func (t *Track) ValidateImport(allowMissing bool) error {
	if strings.TrimSpace(t.Title) == "" {
		err := fmt.Errorf("track title cannot be empty")
		return err
	}
